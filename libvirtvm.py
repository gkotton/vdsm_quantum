...
class NetworkInterfaceDevice(vm.Device):
    def __init__(self, conf, log, **kwargs):
        ...
        self._customize()
        if hasattr(self, 'q_plugin'):
            self.quantum = quantum.Quantum(log, self.q_plugin, self.q_network_id, 
                                           self.q_port_id, self.q_attachment_id)
            self.quantum_network_create()
    ...
    def quantum_network_create(self):
        self.quantum.vifAdd(self.conf['vmId'], self.macAddr)
    ...
    def getXML(self):
        ...
        if hasattr(self, 'q_plugin'):
            ...
            iface.setAttribute('type', 'ethernet')
            m = doc.createElement('mac')
            m.setAttribute('address', self.macAddr)
            iface.appendChild(m)
            m = doc.createElement('target')
            m.setAttribute('dev', self.quantum.vifDeviceName())
            iface.appendChild(m)
...
class LibvirtVm(vm.Vm):
    ...
    def releaseVm(self):
        ...
        for nic in self._devices[vm.NIC_DEVICES]:
            ...
            #removing quantum specific attributes
            if hasattr(nic, 'q_plugin'):
                nic.quantum.vifDelete()
